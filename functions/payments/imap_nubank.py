...
def _check_imap_for_payments(email_address: str, password: str) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        # Buscar apenas emails NÃO LIDOS do Nubank das últimas 2 horas
        from datetime import datetime, timedelta
        import logging as _log
        _imap_log = _log.getLogger("eclipse_store.payments.imap_nubank")
        ...
        try:
            status, messages = mail.search(
                None,
                '(UNSEEN SINCE ' + since_str + ' FROM "todomundo@nubank.com.br")',
            )
            if status == "OK" and messages[0]:
                ...
                try:
                    ...
                    pass
                mail_ids = raw_ids[:50]
                _imap_log.debug(f"Nubank IMAP: {len(mail_ids)} email(s) não lidos encontrados (últimas 2h)")
            else:
                _imap_log.debug("Nubank IMAP: nenhum email não lido encontrado nas últimas 2h")
        except Exception as e:
            _imap_log.warning(f"Nubank IMAP: erro na busca IMAP — {e}")
        ...
        # Processar apenas os primeiros 50 emails mais recentes
        nubank_count = 0
        pix_count = 0
