...
class PaymentWebSocketClient:
    ...
    async def connect(self):
        ...
        try:
            ...
            # Se URL é localhost (servidor local não configurado), não conectar
            if "localhost" in self.server_url or "127.0.0.1" in self.server_url:
                logger.debug(f"API de Pagamentos local não configurada ({self.server_url}), ignorando.")
                return
            ...
            logger.debug(f"Erro ao conectar à API de Pagamentos: {e}")
    ...
    async def authenticate(self, bot_id: str, bot_token: str):
        ...
        try:
            ...
            logger.debug(f"Erro ao autenticar: {e}")
