        ...
        for backup in backups_pagina:
            ...
            containers.append(
                disnake.ui.Container(
                    ...
                    disnake.ui.TextDisplay(
                        f"\n**Estrutura:**\n"
                        f"{emoji.textc} Canais: {str(backup.get('canais',0)).ljust(4)}      {emoji.dir} Categorias: {str(backup.get('categorias',0)).ljust(4)}\n"
                        f"{emoji.role} Cargos: {str(backup.get('cargos',0)).ljust(4)}     {emoji.reaction} Emojis: {str(backup.get('emojis',0)).ljust(4)}\n"
                        f"{emoji.flag} Stickers: {str(backup.get('stickers',0)).ljust(4)}   {emoji.message if hasattr(emoji, 'message') else '💬'} Mensagens: {str(backup.get('mensagens',0)).ljust(4)}\n"
                        f"{emoji.members} Membros: {str(backup.get('membros', 0)).ljust(4)}"
                    ),
                    disnake.ui.Separator(),
                    disnake.ui.ActionRow(
                        disnake.ui.Button(
