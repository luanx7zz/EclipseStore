...
class AdicionarEstoqueModal(disnake.ui.Modal):
    ...
    async def callback(self, inter: disnake.ModalInteraction):
        ...
        try:
            ...
            # Notificação de canal de restock (novo recurso)
            try:
                current_stock = len(items)
                prev_stock = 0
                from functions.restock_notifier import send_restock_notification
                asyncio.create_task(
                    send_restock_notification(
                        ...
                        total_count=current_stock,
                        prev_count=prev_stock,
                    )
                )
            except Exception:
                pass
