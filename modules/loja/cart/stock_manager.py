...
class StockManager:
    ...
    def add_stock_items(product_id: str, campo_id: str, items: List[str]):
        """Adiciona itens ao estoque e retorna (should_notify, prev_count, total_count)"""
        stock = StockManager._load_stock()
        
        # Contar estoque anterior
        prev_count = 0
        had_stock = False
        if product_id in stock and campo_id in stock[product_id]:
            ...
            if isinstance(existing_stock, list):
                prev_count = len(existing_stock)
                had_stock = prev_count > 0
        ...
        total_count = len(stock[product_id][campo_id])
        
        # Salvar
        StockManager._save_stock(stock)
        
        # Notificar quando não havia estoque antes e agora há
