...
from functions.payments._base import _get_base_url as _get_pay_api_url
...
async def _build_cart_message(cart: Dict[str, Any], thread: disnake.Thread, mode: str) -> disnake.Message:
    ...
    def _get_available_payment_method_keys() -> list[str]:
        ...
        providers_coming_soon = []  # Todos os gateways implementados
        all_methods = {
            "pix": {
...
async def create_checkout(
    ...
    # ── Flash Sale: aplicar desconto automático se houver venda ativa ──
    try:
        from modules.loja.flash_sale.cog import apply_flash_sale_price
        _flash_price, _flash_sale = apply_flash_sale_price(product_id, price)
        if _flash_sale:
            price = _flash_price
...
async def _handle_payment_approved(cart_id: str, bot):
    ...
    try:
        ...
        # --- IDEMPOTÊNCIA: evitar dupla entrega em caso de race condition ---
        current_status = cart.get("status", "")
        if current_status in ("approved", "processing"):
            print(f"[CHECKOUT] Carrinho {cart_id} já está com status '{current_status}', ignorando chamada duplicada.")
            return
        
        # Marcar como "processing" imediatamente para bloquear chamadas concorrentes
        cart["status"] = "processing"
        cart["processing_started_at"] = int(datetime.utcnow().timestamp())
        loja_data["carts"][cart_id] = cart
        db.save_document("loja_data", loja_data)
        # --- FIM IDEMPOTÊNCIA ---
            ...
            if user and items:
                    try:
                        ...
                        for item in items:
                            ...
                            # ── Hook: verificar gift pendente e registrar afiliado ──────────
                            try:
                                from modules.loja.gifts.cog import _load_gifts, _save_gifts
                                from modules.loja.affiliates.cog import register_affiliate_sale
                                gifts_data = _load_gifts()
                                gift_key = f'pending_{user.id}_{product_id}_{campo_id}'
                                pending_gift = gifts_data['gifts'].get(gift_key)
                                if pending_gift:
                                    pending_gift['purchase_id'] = purchase_id
                                    pending_gift['items'] = items_delivered
                                    pending_gift['campo_name'] = campo_name
                                    gifts_data['gifts'][gift_key] = pending_gift
                                    _save_gifts(gifts_data)
                                    import asyncio
                                    from modules.loja.gifts.cog import deliver_gift
                                    asyncio.create_task(deliver_gift(bot, gift_key))
                            except Exception as _gift_exc:
                                print(f'[Gift] Erro: {_gift_exc}')
                            try:
                                affiliate_code = cart.get('affiliate_code')
                                if affiliate_code:
                                    from modules.loja.affiliates.cog import validate_affiliate_code, register_affiliate_sale
                                    aff_uid = validate_affiliate_code(affiliate_code)
                                    if aff_uid:
                                        register_affiliate_sale(aff_uid, str(user.id), product_name, item_final_price, purchase_id)
                            except Exception as _aff_exc:
                                print(f'[Affiliate] Erro: {_aff_exc}')
                            # ─────────────────────────────────────────────────────────────────
