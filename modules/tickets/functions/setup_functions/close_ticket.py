...
async def close_ticket(bot: commands.Bot, channel: disnake.TextChannel, closed_by: disnake.Member, reason: str = None, inter: disnake.Interaction = None):
    ...
    if final_transcript_url and closure_log_message:
        # Criar embed com a URL final
        embed = disnake.Embed(
            title="📄 Transcript Final do Ticket",
            description=f"[Clique aqui para visualizar o transcript]({final_transcript_url})",
        ...
        embed.set_footer(text="Transcript final hospedado com segurança")
        
        try:
            await closure_log_message.reply(embed=embed)
        except Exception as e:
            print(f"[TRANSCRIPT] Erro ao enviar embed final nos logs: {e}")
