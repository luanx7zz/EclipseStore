...
from .edit_configs import (
    edit_configs_components,
    edit_configs_embed,
    handle_edit_configs_button,
)
...
class TicketConfigCog(commands.Cog):
    ...
    async def ticket_button_listener(self, inter: disnake.MessageInteraction):
                ...
                try:
                    ...
                    if mode == "embed":
                        await inter.edit_original_message(
                            content=None,
                            embed=disnake.Embed(description=f"❌ Erro ao carregar painel. Tente novamente.", color=0xe74c3c),
                            components=[]
                        )
                    else:
                        await inter.edit_original_message(
                            components=[disnake.ui.Container(disnake.ui.TextDisplay(f"❌ Erro ao carregar painel. Tente novamente."))]
                        )
                except Exception:
                    pass
        ...
        elif custom_id.startswith("EditConfigs_"):
            await handle_edit_configs_button(inter, custom_id)
