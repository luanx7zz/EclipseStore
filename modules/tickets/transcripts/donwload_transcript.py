...
async def send_transcript_to_dm(interaction: disnake.ApplicationCommandInteraction, transcript_file: disnake.File):
    ...
    try:
        # 1. Verificar se já existe um link em cache para este canal
        transcript_url = get_cached_link(interaction.channel.id)
        
        if not transcript_url:
            # 2. Se não houver cache, fazer upload para a API
            ...
            # 3. Salvar no cache se o upload for bem-sucedido
            if transcript_url:
                save_link_to_cache(interaction.channel.id, transcript_url)
        
        if transcript_url:
            # Enviar com URL (seja do cache ou recém-gerada)
            embed = disnake.Embed(
