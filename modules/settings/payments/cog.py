...
class PaymentProviderModal(disnake.ui.Modal):
    def __init__(self, provider_key: str, categoria: str = None):
        ...
        if provider_key == "mercado_pago":
            ...
            nubank_entry = entry.get("nubank_imap") or entry if provider_key == "nubank" else {}
            components.append(
                disnake.ui.Label(
                    text="Email Gmail",
                    component=disnake.ui.TextInput(
                    ...
                    description="Email Gmail vinculado ao Nubank (para receber comprovantes).",
                )
            )
            components.append(
                disnake.ui.Label(
                    text="Senha de App Gmail",
                    component=disnake.ui.TextInput(
                        placeholder="xxxx xxxx xxxx xxxx (16 caracteres)",
                        custom_id="nubank_password",
                        style=disnake.TextInputStyle.short,
                        required=False,
                        value=str(entry.get("password") or ""),
                    ),
                    description="Senha de app gerada em myaccount.google.com > Segurança > Senhas de app.",
                )
            )
            components.append(
                disnake.ui.Label(
                    ...
                    description="Sua chave PIX cadastrada no Nubank.",
                )
            )
            components.append(
                disnake.ui.Label(
                    ...
                    description="Tipo da chave PIX do Nubank.",
                )
            )
    ...
    async def callback(self, inter: disnake.ModalInteraction):
        ...
        if self.provider_key == "efibank":
            ...
            existing_nubank = ConfigurarPagamentos._load_config().get("nubank") or {}
            have_email = bool(nubank_email or existing_nubank.get("email"))
            have_pass = bool(nubank_password or existing_nubank.get("password"))
            have_pix = bool(nubank_pix_key or existing_nubank.get("pix_key"))
            configured = bool(have_email and have_pass and have_pix)
            if enabled and not configured:
                enabled = False
                error_text = "Para ativar o Nubank IMAP, preencha o Email Gmail, Senha de App e Chave PIX."
        ...
        if self.provider_key == "mercado_pago":
            ...
            if nubank_email is not None:
                entry["email"] = nubank_email.strip()
            if nubank_password is not None:
                entry["password"] = nubank_password.strip()
            if nubank_pix_key is not None:
                entry["pix_key"] = nubank_pix_key.strip()
