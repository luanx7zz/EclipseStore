...
class Cloud(commands.Cog):
    ...
    async def _definir_preferencias(self, inter: disnake.MessageInteraction):
        ...
        notif_vendas = cloud_config.get("notify_sales", True)
        notif_gifts = cloud_config.get("notify_gifts", True)
        auto_entrega = cloud_config.get("auto_delivery", False)

        resumo = (
            f"{emoji.on if notif_vendas else emoji.off} **Notif. de Vendas:** `{'Ativado' if notif_vendas else 'Desativado'}`\n"
            f"{emoji.on if notif_gifts else emoji.off} **Notif. de Gifts:** `{'Ativado' if notif_gifts else 'Desativado'}`\n"
            f"{emoji.on if auto_entrega else emoji.off} **Entrega Automática:** `{'Ativado' if auto_entrega else 'Desativado'}`"
        )

        if mode == "embed":
            embed = disnake.Embed(
                title="ZProCloud > Preferências",
                description=f"Configure as preferências do SyncCloud.\n\n{resumo}"
            )
            if primary_color_hex:
                embed.color = disnake.Colour(int(primary_color_hex.replace("#", ""), 16))
            components = [
                disnake.ui.ActionRow(
                    disnake.ui.Button(label="Notif. Vendas", style=disnake.ButtonStyle.green if notif_vendas else disnake.ButtonStyle.grey, emoji=emoji.on if notif_vendas else emoji.off, custom_id="CloudPref_ToggleNotifVendas"),
                    disnake.ui.Button(label="Notif. Gifts", style=disnake.ButtonStyle.green if notif_gifts else disnake.ButtonStyle.grey, emoji=emoji.on if notif_gifts else emoji.off, custom_id="CloudPref_ToggleNotifGifts"),
                    disnake.ui.Button(label="Entrega Auto", style=disnake.ButtonStyle.green if auto_entrega else disnake.ButtonStyle.grey, emoji=emoji.on if auto_entrega else emoji.off, custom_id="CloudPref_ToggleAutoEntrega"),
                ),
                disnake.ui.ActionRow(
            ...
            components = [
                disnake.ui.Container(
                    disnake.ui.TextDisplay(f"# Painel > ZProCloud > Preferências\n\n{resumo}"),
    ...
    async def on_button_click(self, inter: disnake.MessageInteraction):
        ...
        match inter.component.custom_id:
            ...
            case "CloudPref_ToggleNotifVendas" | "CloudPref_ToggleNotifGifts" | "CloudPref_ToggleAutoEntrega":
                cloud_config = db.get_document("cloud_data") or {}
                key_map = {
                    "CloudPref_ToggleNotifVendas": "notify_sales",
                    "CloudPref_ToggleNotifGifts": "notify_gifts",
                    "CloudPref_ToggleAutoEntrega": "auto_delivery",
                }
                key = key_map.get(inter.component.custom_id)
                if key:
                    cloud_config[key] = not cloud_config.get(key, True if key != "auto_delivery" else False)
                    db.save_document("cloud_data", cloud_config)
                await self._definir_preferencias(inter)
